...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      // Check the scheduling policy of the process and schedule accordingly
      if(p->policy==-1){
        struct proc *nxt_proc = p;
        p=nxt_proc;
      }
      else if (p->policy == 0) // EDF
      {
        struct proc *min_deadline_proc = p;
        for(struct proc *p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++){
          ...
          if ( p1->deadline + p1->start_time < min_deadline_proc->deadline + min_deadline_proc->start_time)
            min_deadline_proc = p1; // find the process with the earliest deadline
        }
        p = min_deadline_proc; // switch to the process with the earliest deadline
      }
      else if (p->policy == 1) // RM
      {
        struct proc *min_period_proc = p;
        for(struct proc *p1 = ptable.proc; p1 < &ptable.proc[NPROC]; p1++){
          if (p1->state != RUNNABLE || p1->policy != 1) // not a runnable RM process
            continue;
          if (p1->rate < min_period_proc->rate)
            min_period_proc = p1; // find the process with the shortest period
          p = min_period_proc; // switch to the process with the shortest period
        }
      }
      ...
      switchuvm(p);
      p->state = RUNNING;
      //cprintf("**** Scheduling process: %d", p->pid);
      swtch(&(c->scheduler), p->context);
      switchkvm();
...
rate_helper(int pid, int rate){
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if (p->pid == pid) {
      // Save the old rate for later return
      old_rate = p->rate;
